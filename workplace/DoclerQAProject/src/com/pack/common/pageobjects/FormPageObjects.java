...
public class FormPageObjects {
	...
	public static WebElement formBtn(WebDriver driver){		 
	 ...
	 public static void waitUntilInputAppears(WebDriver driver){
		 ...
		 wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hello-input")));		 
	 }
	 
	 public static void waitUntilHelloTextAppears(WebDriver driver){
		 WebDriverWait wait = new WebDriverWait(driver, 15);
		 wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hello-text")));		 
	 }
	 
	 public static boolean verifyFormPageUrl(WebDriver driver) {
			String currentURL = driver.getCurrentUrl();
			String formPageURL ="http://uitest.duodecadits.com/form.html";
			return currentURL.contains(formPageURL);
	 }
	 
	 public static boolean verifyHelloPage(WebDriver driver) {
			String currentURL = driver.getCurrentUrl();
			return currentURL.contains("http://uitest.duodecadits.com/hello.html?myName") 
					& helloText(driver).isDisplayed();
	 }
	 
	 public static boolean verifyFormBtnActive(WebDriver driver) {
			return GeneralObjects.currentActiveBtnId(driver).contains("form");
	 }
